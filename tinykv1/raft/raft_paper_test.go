...
func testUpdateTermFromMessage(t *testing.T, state StateType) {
	...
	if r.Term != 2 {
		...
		//TestCandidateUpdateTermFromMessage2AA
		//raft_paper_test.go:67: term = 1, want 2
	}
	if r.State != StateFollower {
		...
		//TestCandidateUpdateTermFromMessage2AA
		//raft_paper_test.go:70: state = StateCandidate, want StateFollower
	}
}
...
func TestLeaderBcastBeat2AA(t *testing.T) {
	...
	msgs := r.readMessages()
	//疑问：为什么返回结果是MessageType_MsgHeartbeat,从哪里来的
	//答：tick（）
...
func testNonleaderStartElection(t *testing.T, state StateType) {
	...
	msgs := r.readMessages()
	sort.Sort(messageSlice(msgs))
	//拦路虎：MessageType_MsgRequestVote 不清楚在哪里发出根本做不下去
	//答：
	wmsgs := []pb.Message{
...
// c) it is unclear about the result tick
// Reference: section 5.2
func TestLeaderElectionInOneRoundRPC2AA(t *testing.T) {
	tests := []struct {
		...
		{1, map[uint64]bool{}, StateLeader}, //没有限制至少三个
	...
	for i, tt := range tests {
		...
		//触发选举流程 通常的调用方式是直接使用 Step 传递一个 MsgHup 类型的信息给 raft 模块处理
		r.Step(pb.Message{From: 1, To: 1, MsgType: pb.MessageType_MsgHup}) //send
		for id, vote := range tt.votes {
			r.Step(pb.Message{From: id, To: 1, Term: r.Term, MsgType: pb.MessageType_MsgRequestVoteResponse, Reject: !vote}) //deal
		}
...
func TestFollowerVote2AA(t *testing.T) {
	tests := []struct {
		...
		{1, 2, true},
		{2, 1, true},
		//如果对方拒绝了本次选举请求， Reject 会被置为 True
	}
	for i, tt := range tests {
		r := newTestRaft(1, []uint64{1, 2, 3}, 10, 1, NewMemoryStorage())
		r.Term = 1
		r.Vote = tt.vote //自己投票自己
		//该消息用于选举过程中请求别的节点把选票投给自己
		// From：发起方， 发起方的 Term: 1
		// To: 接收方   接收方信息 r.Term
...
func TestCandidateFallback2AA(t *testing.T) {
	...
	for i, tt := range tests {
		...
		//MessageType_MsgHup -->StateCandidate
		if r.State != StateCandidate {
