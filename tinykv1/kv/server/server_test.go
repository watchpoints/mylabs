...
func TestRawGet1(t *testing.T) {
	...
	assert.Nil(t, err)
	log.Println("TestRawGet1 key=", req.Key, "value=", resp.Value)
...
func TestRawGetNotFound1(t *testing.T) {
	...
	resp, err := server.RawGet(nil, req)
	assert.Nil(t, err) // 如果是null，应该返回错误
...
func TestRawPut1(t *testing.T) {
	...
	assert.Nil(t, err) //Expected nil, but got: Key not found
...
func TestRawGetAfterRawDelete1(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.True(t, resp.NotFound) // Should be true
}
...
func TestRawDelete1(t *testing.T) {
	...
	assert.Equal(t, nil, err) //expected: <nil>
...
func TestRawScan1(t *testing.T) {
	...
	//小王提问：这个是什么意思，StartKey 从一个key 开始连续3个
