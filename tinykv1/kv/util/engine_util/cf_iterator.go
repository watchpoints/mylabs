...
//https://github.com/Connor1996/badger#iterating-over-keys
//在创建时候不知道cf是什么
type BadgerOverIterator struct {
	iter *badger.Iterator
}

func NewOverIterator(txn *badger.Txn) *BadgerIterator {
	return &BadgerIterator{
		iter: txn.NewIterator(badger.DefaultIteratorOptions),
	}
}

func (it *BadgerOverIterator) Item() DBItem {
	return &CFItem{
		item: it.iter.Item(),
	}
}

func (it *BadgerOverIterator) Valid() bool { return it.iter.Valid() }

func (it *BadgerOverIterator) Close() {
	it.iter.Close()
}

func (it *BadgerOverIterator) Next() {
	it.iter.Next()
}

func (it *BadgerOverIterator) Seek(key []byte) {
	it.iter.Seek(key)
}

func (it *BadgerOverIterator) Rewind() {
	it.iter.Rewind()
}
